...
class StudentsController extends Controller
{
    ...
    public function getAllDataByParent($parent_id){
        ...
        foreach ($data AS $k=> $student){
            if($student->crm_id){
                ...
                $contract_last =u::firstCRM("SELECT id, enrolment_last_date FROM contracts WHERe student_id= ".$student->crm_id." AND type>0 ORDER BY count_recharge DESC LIMIT 1");
                ...
                $data[$k]->student_active = data_get($crm_student_info, 'staus') ? 1 : 0;
            }
        }
    ...
    public static function updateStudentCRM($student_id){
        ...
        if(env('APP_ENV', 'staging')=='production'){
            $url = sprintf('%s/api/leads-update-student-info', 'https://crm.logiclab.vn/');
        }else{
            $url = sprintf('%s/api/leads-update-student-info', 'https://staging.logiclab.vn/');
        }
        $res = curl::curl($url, $method,[],$data);
        u::logRequest($url,$method,[],$data,$res,'log_request_outbound');
        $res = json_decode($res);

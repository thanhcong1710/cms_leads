...
class ParentsController extends Controller
{
    ...
    public function list(Request $request)
    {
        ...
        $start_date_assign = isset($request->start_date_assign) ? $request->start_date_assign : '';
        $end_date_assign = isset($request->end_date_assign) ? $request->end_date_assign : '';
        ...
        if(!empty($students)){
            ...
            if( $owner_info){
                ...
                foreach($students AS $student){
                    if(data_get($student, 'crm_id')){
                        u::queryCRM("UPDATE term_student_user SET ec_id= ".data_get($crm_ec_id, 'id')." WHERE student_id =".data_get($student, 'crm_id'));
                    }
                }
            }
        }
    ...
    public function assignList(Request $request)
    {
        ...
        foreach($list_parent_info AS $k=>$row){
            ...
            // if(!empty($students)){
            //     $owner_info = u::first("SELECT hrm_id FROM users WHERE id=".$owner_id);
            //     if( $owner_info){
            //         $crm_ec_id = u::firstCRM("SELECT id FROM users WHERE hrm_id = ".data_get($owner_info, 'hrm_id'));
            //         foreach($students AS $student){
            //             if(data_get($student, 'crm_id')){
    ...
    public static function processParentLockById($parent_id){
        ...
        u::query("UPDATE cms_parents AS p SET p.is_lock = 1 WHERE p.id=$parent_id ");

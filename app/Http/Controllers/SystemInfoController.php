...
class SystemInfoController extends Controller
{
    ...
    public function getAllSourceDetail(Request $request){
        $cond= $request->user()->branch_id && !$request->user()->hasRole('admin') && !$request->user()->hasRole('Supervisor') ? " AND (branch_id =".$request->user()->branch_id." OR branch_id IS NULL OR branch_id=0)" :"";

...
{
    protected $table = 'cms_parent_logs';
    public static function logAssign($parent_id,$pre_owner_id,$owner_id,$creator_id,$overwrite = false){
        ...
        u::insertSimpleRow(array(
            ...
            'status'=> in_array($creator_id,[39,40])? 0 : 1
        ),'cms_parent_logs');
    }
    public static function logAssignList($list_parent_info,$owner_id,$creator_id){
        $created_at = date('Y-m-d H:i:s');
        $owner_info = u::first("SELECT name,hrm_id FROM users WHERE id = $owner_id");
        $query = "INSERT INTO cms_parent_logs (`parent_id`,`content`,creator_id,created_at) VALUES ";
    ...
    public static function logStatus($parent_id,$pre_status,$status,$creator_id){
        $arr_status = array(
            ...
            '5'=>'KH không nghe máy',
            '6'=>'KH đồng ý đặt lịch checkin',
            '7'=>'KH đã đến checkin',
            '8'=>'KH đã mua gói phí',
            '9'=>'KH không có nhu cầu',
            '10'=>'KH không tiềm năng',
        ...
        foreach($data_update AS $key=>$row){
            if($row!=$pre_parent_info->$key){
                if($key=='name'){
                    ...
                    $pre_province_info = u::first("SELECT name FROM cms_provinces WHERE id =".(int)$pre_parent_info->$key);
                    $province_info = u::first("SELECT name FROM cms_provinces WHERE id =".(int)$row);
                    ...
                    $pre_district_info = u::first("SELECT name FROM cms_districts WHERE id =".(int)$pre_parent_info->$key);
                    $district_info = u::first("SELECT name FROM cms_districts WHERE id =".(int)$row);
                    ...
                    $pre_job_info = u::first("SELECT title FROM cms_jobs WHERE id =".(int)$pre_parent_info->$key);
                    $job_info = u::first("SELECT title FROM cms_jobs WHERE id =".(int)$row);
